...
export default function Home() {
  ...
  return (
    <main className={`app-shell ${mobileNavOpen ? 'nav-open' : ''} ${selected ? 'detail-open' : ''}`}>
      ...
      {composerOpen && <Compose seed={composeSeed} onClose={() => { setComposerOpen(false); setComposeSeed({}); }} onDraftDeleted={() => { setDrafts((current) => current.filter((draft) => draft.id !== composeSeed.draftId)); }} onSent={() => { setComposerOpen(false); setComposeSeed({}); window.location.reload(); }} />}
    </main>
  );
}

function Compose({ seed, onClose, onDraftDeleted, onSent }: { seed: ComposeSeed; onClose: () => void; onDraftDeleted: () => void; onSent: () => void }) {
  ...
  useEffect(() => {
    setTo(seed.to ?? ''); setSubject(seed.subject ?? ''); setBody(seed.body ?? ''); setDraftId(seed.draftId ?? ''); setStatus('');
  ...
  return <div className="overlay" role="dialog" aria-modal="true" aria-label="Compose email">
    <form className="composer-window" onSubmit={submit}>
      ...
      <div className="composer-foot"><div className="composer-status"><span className={status === 'Saved' || status === 'Message sent' ? 'success' : 'status-text'}>{status}</span>{draftId && status === 'Saved' && <small>Draft saved</small>}</div><div className="composer-actions">{draftId && <button className="delete-draft-button" type="button" onClick={deleteDraft} disabled={busy}>Delete draft</button>}<button className="send-button" type="submit" disabled={busy}>{busy ? 'Sending…' : 'Send'} <Send size={15} /></button></div></div>
    </form>
  </div>;
}
