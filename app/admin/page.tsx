...
import { Activity, AlertTriangle, ArrowUpRight, Database, Mail, ShieldCheck, Smartphone, Users, WalletCards, Wifi } from 'lucide-react';
...
type Overview = {
  ...
  metrics: { inbound: number; outbound: number; nonSentOutbound: number; browserNotificationUsers: number };
  ...
  return (
    <main className={styles.shell}>
      ...
      <section className={styles.metricGrid}>
        ...
        <div className={styles.metricCard}><Smartphone size={18} /><span>Push-enabled users</span><strong>{data.metrics.browserNotificationUsers}</strong><small>Browser notification users</small></div>
      </section>

      <section className={styles.twoCol}>
        <div className={styles.panel}><div className={styles.panelHead}><div><p className={styles.eyebrow}>Incidents</p><h2>Operational incidents</h2></div><span className={styles.counter}>{activeIncidents.length} open</span></div>{activeIncidents.length === 0 ? <div className={styles.empty}><ShieldCheck size={22} /><strong>No active incidents</strong><span>The monitoring store is ready for automatic incident detection.</span></div> : activeIncidents.map((incident) => <div className={styles.listRow} key={incident.id}><div><strong>{incident.title}</strong><span>{incident.incident_key} · {incident.component ?? 'system'}</span></div><b className={incident.severity === 'P1' ? styles.danger : styles.badge}>{incident.severity}</b></div>)}</div>
        <div className={styles.panel}><div className={styles.panelHead}><div><p className={styles.eyebrow}>User reports</p><h2>Reported problems</h2></div><span className={styles.counter}>{openReports.length} open</span></div>{openReports.length === 0 ? <div className={styles.empty}><Activity size={22} /><strong>No outstanding reports</strong><span>User-submitted diagnostics will appear here once reporting is enabled.</span></div> : openReports.map((report) => <div className={styles.listRow} key={report.id}><div><strong>{report.description || report.action || 'Reported problem'}</strong><span>{report.report_key} · {report.mailbox || 'mailbox unknown'}</span></div><b className={styles.badge}>{report.status}</b></div>)}</div>
      </section>
