...
export async function GET() {
  try {
    ...
    const [events, activeIncidents, reports, outboundFailures, inbound, outbound] = await Promise.all([
    ...
    const missingSchema = resultSets.find((result) => result.error?.code === 'PGRST205' || result.error?.code === '42P01');
    const realError = resultSets.find((result) => result.error && result.error.code !== 'PGRST205' && result.error.code !== '42P01');
