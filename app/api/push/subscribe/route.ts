...
async function getUser() {
  ...
  if (!user?.id || !email) return { error: NextResponse.json({ error: 'Authentication required.' }, { status: 401 }) };
  if (!email.endsWith(`@${MAIL_DOMAIN}`)) return { error: NextResponse.json({ error: 'Your account is not configured as a Waste2Light mailbox.' }, { status: 403 }) };
  return { user, email };
}
