...
function formatSize(size: number) {
  ...
  return `${Math.round(size / (1024 * 1024)) / 10} MB`;
}
...
function installComposer() {
  ...
  const render = () => {
    list.innerHTML = state.uploads.map((item) => `
      <span class="attachment-chip">
        ...
        <button type="button" data-attachment-remove="${item.id}" aria-label="Remove attachment">×</button>
      </span>`).join('');
  ...
  composer.addEventListener('submit', async (event) => {
    if (!state.uploads.length || state.busy) return;
  ...
  closeButton?.addEventListener('click', () => {
    if (state.uploads.length) void supabase.storage.from(BUCKET).remove(state.uploads.map((item) => item.path));
  }, true);
}
